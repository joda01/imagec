...
static int _TIFFVSetField(TIFF *tif, ttag_t tag, va_list ap)
{
  ...
  switch(tag) {
    ...
    case TIFFTAG_ORIENTATION:
      ...
      if(v < ORIENTATION_TOPLEFT || ORIENTATION_LEFTBOT < v) {
        TIFFWarningExt(tif->tif_clientdata, tif->tif_name, "Bad value %lu for \"%s\" tag ignored", v,
    ...
    case TIFFTAG_SAMPLEFORMAT:
      ...
      if(td->td_sampleformat == SAMPLEFORMAT_COMPLEXINT && td->td_bitspersample == 32 &&
         tif->tif_postdecode == _TIFFSwab32BitData)
        tif->tif_postdecode = _TIFFSwab16BitData;
      else if((td->td_sampleformat == SAMPLEFORMAT_COMPLEXINT || td->td_sampleformat == SAMPLEFORMAT_COMPLEXIEEEFP) &&
              td->td_bitspersample == 64 && tif->tif_postdecode == _TIFFSwab64BitData)
        tif->tif_postdecode = _TIFFSwab32BitData;
      break;
    ...
    case TIFFTAG_SUBIFD: /* SubIFD array with 64-bit offsets */
      if((tif->tif_flags & TIFF_INSUBIFD) == 0) {
        ...
        _TIFFsetLong8Array((toff_t **) &td->td_subifd, va_arg(ap, toff_t *), (long) td->td_nsubifd);
      } else {
    ...
    case TIFFTAG_TRANSFERFUNCTION:
      ...
      for(i = 0; i < v; i++)
        _TIFFsetShortArray(&td->td_transferfunction[i], va_arg(ap, uint16 *), 1L << td->td_bitspersample);
      break;
    ...
    default: {
      ...
      if(fip == NULL || fip->field_bit != FIELD_CUSTOM) {
        TIFFErrorExt(tif->tif_clientdata, module, "%s: Invalid %stag \"%s\" (not supported by codec)", tif->tif_name,
                     isPseudoTag(tag) ? "pseudo-" : "", _TIFFFieldWithTag(tif, tag)->field_name);
        status = 0;
        break;
      }
      ...
      if(tv == NULL) {
        ...
        new_customValues =
            (TIFFTagValue *) _TIFFrealloc(td->td_customValues, sizeof(TIFFTagValue) * td->td_customValueCount);
        if(!new_customValues) {
          TIFFErrorExt(tif->tif_clientdata, module, "%s: Failed to allocate space for list of custom values",
                       tif->tif_name);
          status = 0;
          goto end;
        }
      ...
      if(tv_size == 0) {
        status = 0;
        TIFFErrorExt(tif->tif_clientdata, module, "%s: Bad field type %d for \"%s\"", tif->tif_name, fip->field_type,
                     fip->field_name);
        goto end;
      }
      ...
      if(fip->field_type == TIFF_ASCII)
        ...
        if((fip->field_passcount || fip->field_writecount == TIFF_VARIABLE || fip->field_writecount == TIFF_VARIABLE2 ||
            fip->field_writecount == TIFF_SPP || tv->count > 1) &&
           fip->field_tag != TIFFTAG_PAGENUMBER && fip->field_tag != TIFFTAG_HALFTONEHINTS &&
           fip->field_tag != TIFFTAG_YCBCRSUBSAMPLING && fip->field_tag != TIFFTAG_DOTRANGE) {
  ...
  return (status);
badvalue:
  TIFFErrorExt(tif->tif_clientdata, module, "%s: Bad value %d for \"%s\"", tif->tif_name, v,
               _TIFFFieldWithTag(tif, tag)->field_name);
  va_end(ap);
  return (0);
badvalue32:
  TIFFErrorExt(tif->tif_clientdata, module, "%s: Bad value %ld for \"%s\"", tif->tif_name, v32,
               _TIFFFieldWithTag(tif, tag)->field_name);
  va_end(ap);
  return (0);
}
...
static int OkToChangeTag(TIFF *tif, ttag_t tag)
{
  const TIFFFieldInfo *fip = _TIFFFindFieldInfo(tif, tag, TIFF_ANY);
  if(!fip) { /* unknown tag */
    TIFFErrorExt(tif->tif_clientdata, "TIFFSetField", "%s: Unknown %stag %u", tif->tif_name,
  ...
  if(tag != TIFFTAG_IMAGELENGTH && (tif->tif_flags & TIFF_BEENWRITING) && !fip->field_oktochange) {
     ...
     */
    TIFFErrorExt(tif->tif_clientdata, "TIFFSetField", "%s: Cannot modify tag \"%s\" while writing", tif->tif_name,
                 fip->field_name);
    return (0);
  }
  return (1);
    ...
    default: {
      ...
      if(fip == NULL || fip->field_bit != FIELD_CUSTOM) {
        TIFFErrorExt(tif->tif_clientdata, "_TIFFVGetField", "%s: Invalid %stag \"%s\" (not supported by codec)",
                     tif->tif_name, isPseudoTag(tag) ? "pseudo-" : "", _TIFFFieldWithTag(tif, tag)->field_name);
        ret_val = 0;
        break;
      }
      ...
      for(i = 0; i < td->td_customValueCount; i++) {
        ...
        if(fip->field_passcount) {
          ...
          *va_arg(ap, void **) = tv->value;
          ...
          if((fip->field_type == TIFF_ASCII || fip->field_readcount == TIFF_VARIABLE ||
              fip->field_readcount == TIFF_VARIABLE2 || fip->field_readcount == TIFF_SPP || tv->count > 1) &&
             fip->field_tag != TIFFTAG_PAGENUMBER && fip->field_tag != TIFFTAG_HALFTONEHINTS &&
             fip->field_tag != TIFFTAG_YCBCRSUBSAMPLING && fip->field_tag != TIFFTAG_DOTRANGE) {
            *va_arg(ap, void **) = tv->value;
            ret_val              = 1;
            ...
            for(j = 0; j < tv->count; j++, val += _TIFFDataSize(tv->info->field_type)) {
              switch(fip->field_type) {
                case TIFF_BYTE:
                case TIFF_UNDEFINED:
...
int TIFFDefaultDirectory(TIFF *tif)
{
  ...
  const TIFFFieldInfo *tiffFieldInfo = _TIFFGetFieldInfo(&tiffFieldInfoCount);
...
static int TIFFAdvanceDirectory(TIFF *tif, toff_t *nextdir, toff_t *off)
{
  ...
  if(isMapped(tif)) {
    ...
    if(!SeekOK(tif, *nextdir) || !ReadOK(tif, &dircount, TIFFDirCntLen(tif))) {
      TIFFErrorExt(tif->tif_clientdata, module, "%s: Error fetching directory count (%d)", tif->tif_name,
                   TIFFGetErrno(tif));
      return (0);
    }
    if(tif->tif_flags & TIFF_SWAB)
    ...
    if(!ReadOK(tif, &nextdiroff, TIFFDirOffLen(tif))) {
      TIFFErrorExt(tif->tif_clientdata, module, "%s: Error fetching directory link (%d)", tif->tif_name,
                   TIFFGetErrno(tif));
      return (0);
    }
  }
 ...
 */
int TIFFSetSubDirectory(TIFF *tif, toff_t diroff)
{
 ...
 */
toff_t TIFFCurrentDirOffset(TIFF *tif)
{
...
int TIFFUnlinkDirectory(TIFF *tif, tdir_t dirn)
{
  ...
  if(!WriteOK(tif, &nextdiroff, TIFFDirOffLen(tif))) {
    TIFFErrorExt(tif->tif_clientdata, module, "Error writing directory link (%d)", TIFFGetErrno(tif));
    return (0);
  }
  /*
