  ...
  for(j = 0; j < value_count; j++) {
    switch(fip->field_type) {
      case TIFF_BYTE:
        ...
        break;
      case TIFF_UNDEFINED:
        fprintf(fd, "0x%x", (unsigned int) ((unsigned char *) raw_data)[j]);
        break;
      case TIFF_SBYTE:
      ...
      case TIFF_LONG8:
        ...
        break;
      case TIFF_SLONG8:
        fprintf(fd, "%" I64FMT "d", ((int64_t *) raw_data)[j]);
        break;
      case TIFF_RATIONAL:
      case TIFF_SRATIONAL:
      case TIFF_FLOAT:
...
void TIFFPrintDirectory(TIFF *tif, FILE *fd, long flags)
{
  ...
  if(TIFFFieldSet(tif, FIELD_IMAGEDIMENSIONS)) {
    fprintf(fd, "  Image Width: %lu Image Length: %lu", (unsigned long) td->td_imagewidth,
  ...
  if(TIFFFieldSet(tif, FIELD_TILEDIMENSIONS)) {
    fprintf(fd, "  Tile Width: %lu Tile Length: %lu", (unsigned long) td->td_tilewidth,
  ...
  if(TIFFFieldSet(tif, FIELD_YCBCRSUBSAMPLING)) {
    ...
    TIFFGetField(tif, TIFFTAG_YCBCRSUBSAMPLING, subsampling + 0, subsampling + 1);
  ...
  */
    ...
    for(i = 0; i < count; i++) {
      ...
      if(fip->field_passcount) {
        ...
        if((fip->field_type == TIFF_ASCII || fip->field_readcount == TIFF_VARIABLE ||
            fip->field_readcount == TIFF_VARIABLE2 || fip->field_readcount == TIFF_SPP || value_count > 1) &&
          ...
          if(TIFFGetField(tif, tag, &raw_data) != 1)
            continue;
        } else if(fip->field_tag != TIFFTAG_PAGENUMBER && fip->field_tag != TIFFTAG_HALFTONEHINTS &&
                  fip->field_tag != TIFFTAG_YCBCRSUBSAMPLING && fip->field_tag != TIFFTAG_DOTRANGE) {
          ...
          tmp       = raw_data;
          mem_alloc = 1;
          if(TIFFGetField(tif, tag, tmp, tmp + _TIFFDataSize(fip->field_type)) != 1) {
            _TIFFfree(raw_data);
            continue;
          }
        }
  ...
  if((flags & TIFFPRINT_STRIPS) && TIFFFieldSet(tif, FIELD_STRIPOFFSETS)) {
    ...
    fprintf(fd, "  %lu %s:\n", (long) td->td_nstrips, isTiled(tif) ? "Tiles" : "Strips");
    for(s = 0; s < td->td_nstrips; s++)
      fprintf(fd, "    %6lu: [%12" I64FMT "u, %8lu]\n", (uint32) s, (toff_t) _TIFFGetOffset(tif, s),
