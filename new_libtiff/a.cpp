...
static int isCCITTCompression(TIFF *tif)
{
  ...
  return (compress == COMPRESSION_CCITTFAX3 || compress == COMPRESSION_CCITTFAX4 || compress == COMPRESSION_CCITTRLE ||
...
int TIFFRGBAImageBegin_64(TIFFRGBAImage *img, TIFF *tif, int stop, char emsg[1024])
{
  ...
  switch(img->photometric) {
    ...
    case PHOTOMETRIC_LOGLUV:
      if(compress != COMPRESSION_SGILOG && compress != COMPRESSION_SGILOG24) {
        sprintf(emsg, "Sorry, LogLuv data must have %s=%d or %d", "Compression", COMPRESSION_SGILOG,
  ...
  img->isContig = !(planarconfig == PLANARCONFIG_SEPARATE && img->samplesperpixel > 1);
  if(img->isContig) {
 ...
 *	SamplesPerPixel == 1
 */
static int gtTileContig(TIFFRGBAImage *img, uint64 *raster, uint32 w, uint32 h)
{
  TIFF *tif                = img->tif;
  ...
  for(row = 0; row < h; row += nrow) {
    ...
    for(col = 0; col < w; col += tw) {
      if(TIFFReadTile(tif, buf, col + img->col_offset, row + img->row_offset, 0, 0) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
      }
 ...
 */
static int gtTileSeparate(TIFFRGBAImage *img, uint64 *raster, uint32 w, uint32 h)
{
  TIFF *tif                  = img->tif;
  ...
  for(row = 0; row < h; row += nrow) {
    ...
    nrow       = (row + rowstoread > h ? h - row : rowstoread);
    for(col = 0; col < w; col += tw) {
      if(TIFFReadTile(tif, p0, col + img->col_offset, row + img->row_offset, 0, 0) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
      }
      if(colorchannels > 1 &&
         TIFFReadTile(tif, p1, col + img->col_offset, row + img->row_offset, 0, 1) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
      }
      if(colorchannels > 1 &&
         TIFFReadTile(tif, p2, col + img->col_offset, row + img->row_offset, 0, 2) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
      }
      if(alpha &&
         TIFFReadTile(tif, pa, col + img->col_offset, row + img->row_offset, 0, colorchannels) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
      }
      ...
      if(col + tw > w) {
        ...
        uint32 npix = w - col;
        fromskew    = tw - npix;
        (*put)(img, raster + y * w + col, col, y, npix, nrow, fromskew, toskew + fromskew, p0 + pos, p1 + pos, p2 + pos,
               (alpha ? (pa + pos) : NULL));
      } else {
        (*put)(img, raster + y * w + col, col, y, tw, nrow, 0, toskew, p0 + pos, p1 + pos, p2 + pos,
               (alpha ? (pa + pos) : NULL));
      }
    }
 ...
 *	SamplesPerPixel == 1
 */
static int gtStripContig(TIFFRGBAImage *img, uint64 *raster, uint32 w, uint32 h)
{
  TIFF *tif                = img->tif;
  tileContigRoutine_64 put = (tileContigRoutine_64) img->put.contig;
 ...
 * We assume that all such images are RGB.
 */
static int gtStripSeparate(TIFFRGBAImage *img, uint64 *raster, uint32 w, uint32 h)
{
  TIFF *tif                  = img->tif;
  tileSeparateRoutine_64 put = (tileSeparateRoutine_64) img->put.separate;
  unsigned char *buf;
  ...
  for(row = 0; row < h; row += nrow) {
    ...
    if(TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, 0), p0,
    ...
    if(alpha) {
      if(TIFFReadEncodedStrip(tif, TIFFComputeStrip(tif, offset_row, colorchannels), pa,
                              ((row + img->row_offset) % rowsperstrip + nrow) * scanline) == (tmsize_t) (-1) &&
         img->stoponerr) {
        ret = 0;
        break;
    ...
    pos = ((row + img->row_offset) % rowsperstrip) * scanline;
    (*put)(img, raster + y * w, 0, y, w, nrow, fromskew, toskew, p0 + pos, p1 + pos, p2 + pos,
           (alpha ? (pa + pos) : NULL));
    y += (flip & FLIP_VERTICALLY ? -(int32) nrow : (int32) nrow);
  }
...
#define PACK_32(r, g, b) ((uint32) (r) | ((uint32) (g) << 8) | ((uint32) (b) << 16) | (0xff << 24))
#define PACK(r, g, b) ((uint64) (r) | ((uint64) (g) << 16) | ((uint64) (b) << 32) | A1)
#define PACK4(r, g, b, a) ((uint64) (r) | ((uint64) (g) << 16) | ((uint64) (b) << 32) | ((uint64) (a) << 48))
#define W2B(v) (((v) >> 16) & 0xffff)
/* TODO: PACKW should have be made redundant in favor of Bitdepth16To8 LUT */
#define PACKW(r, g, b) ((uint64) W2B(r) | ((uint64) W2B(g) << 16) | ((uint64) W2B(b) << 32) | A1)
#define PACKW4(r, g, b, a) \
  ((uint64) W2B(r) | ((uint64) W2B(g) << 16) | ((uint64) W2B(b) << 32) | ((uint64) W2B(a) << 48))

#define DECLAREContigPutFunc(name)                                                                         \
  static void name(TIFFRGBAImage *img, uint64 *cp, uint32 x, uint32 y, uint32 w, uint32 h, int32 fromskew, \
                   int32 toskew, unsigned char *pp)
...
DECLAREContigPutFunc(putRGBcontig8bittile)
{
  ...
  while(h-- > 0) {
    UNROLL8(w, NOP, *cp++ = PACK(pp[0] << 8, pp[1] << 8, pp[2] << 8); pp += samplesperpixel);
    cp += toskew;
    pp += fromskew;
  }
}
...
DECLAREContigPutFunc(putRGBAAcontig8bittile)
{
  ...
  fromskew *= samplesperpixel;
  while(h-- > 0) {
    UNROLL8(w, NOP, *cp++ = PACK4(pp[0] << 8, pp[1] << 8, pp[2] << 8, pp[3] << 8); pp += samplesperpixel);
    cp += toskew;
    pp += fromskew;
  }
}
...
DECLAREContigPutFunc(putRGBcontig8bitCMYKtile)
{
  ...
  fromskew *= samplesperpixel;
  while(h-- > 0) {
    UNROLL8(w, NOP, k = 255 - pp[3]; r = (k * (255 - pp[0])); g = (k * (255 - pp[1])); b = (k * (255 - pp[2]));
...
#define DECLARESepPutFunc(name)                                                                            \
  static void name(TIFFRGBAImage *img, uint64 *cp, uint32 x, uint32 y, uint32 w, uint32 h, int32 fromskew, \
                   int32 toskew, unsigned char *r, unsigned char *g, unsigned char *b, unsigned char *a)
...
static int initYCbCrConversion(TIFFRGBAImage *img)
{
  ...
  if(img->ycbcr == NULL) {
    img->ycbcr = (TIFFYCbCrToRGB *) _TIFFmalloc(TIFFroundup_32(sizeof(TIFFYCbCrToRGB), sizeof(long)) +
...
static tileContigRoutine_64 initCIELabConversion(TIFFRGBAImage *img)
{
...
static int makebwmap(TIFFRGBAImage *img)
{
  ...
  img->BWmap = (uint32 **) _TIFFmalloc(256 * sizeof(uint32 *) + (256 * nsamples * sizeof(uint64)));
  if(img->BWmap == NULL) {
...

{
  ...
  for(i_row = 0; i_row < read_ysize; i_row++) {
    memmove(raster + (tile_ysize - i_row - 1) * tile_xsize, raster + (read_ysize - i_row - 1) * read_xsize,
            read_xsize * sizeof(uint64));
    _TIFFmemset(raster + (tile_ysize - i_row - 1) * tile_xsize + read_xsize, 0,
                sizeof(uint64) * (tile_xsize - read_xsize));
  }

  for(i_row = read_ysize; i_row < tile_ysize; i_row++) {
    _TIFFmemset(raster + (tile_ysize - i_row - 1) * tile_xsize, 0, sizeof(uint64) * tile_xsize);
  }
