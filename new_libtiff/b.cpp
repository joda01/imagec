...
#define DECLAREContigPutFunc(name)                                                                         \
  static void name(TIFFRGBAImage *img, uint32 *cp, uint32 x, uint32 y, uint32 w, uint32 h, int32 fromskew, \
...
DECLAREContigPutFunc(putRGBAAcontig16bittile)
{
  ...
  while(h-- > 0) {
    for(x = w; x-- > 0;) {
      *cp++ = PACK4(img->Bitdepth16To8[wp[0]], img->Bitdepth16To8[wp[1]], img->Bitdepth16To8[wp[2]],
...
DECLAREContigPutFunc(putRGBcontig8bitCMYKtile)
{
  ...
  fromskew *= samplesperpixel;
  while(h-- > 0) {
    UNROLL8(w, NOP, k = 255 - pp[3]; r = (k * (255 - pp[0])) / 255; g = (k * (255 - pp[1])) / 255;
            b = (k * (255 - pp[2])) / 255; *cp++ = PACK(r, g, b); pp += samplesperpixel);
    cp += toskew;
    pp += fromskew;
  }
}
...
DECLARESepPutFunc(putRGBAAseparate16bittile)
{
  ...
  while(h-- > 0) {
    for(x = 0; x < w; x++)
      *cp++ = PACK4(img->Bitdepth16To8[*wr++], img->Bitdepth16To8[*wg++], img->Bitdepth16To8[*wb++],
 ...
 */
#ifdef notdef
static void putcontig8bitYCbCrGenericTile(TIFFRGBAImage *img, uint32 *cp, uint32 x, uint32 y, uint32 w, uint32 h,
                                          int32 fromskew, int32 toskew, unsigned char *pp, int h_group, int v_group)
