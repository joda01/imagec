                                            ...
                                            const joda::settings::json::AnalyzeSettings &settings) -> std::string
{
  if(!mProcessors.empty()) {
    throw std::runtime_error("There is still an analysis in progess!");
  }

  std::shared_ptr<joda::processor::ImageProcessorBase> processor;
  switch(settings.getPipeline()) {
    case joda::settings::json::AnalyzeSettings::Pipeline::NUCLEUS_COUNT:
      processor = std::make_shared<joda::processor::ImageProcessor<::joda::pipeline::NucleusCounter>>(
          inputFolder, inputFolder + "/result");
      break;
