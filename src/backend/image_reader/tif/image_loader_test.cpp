...
{
  auto prop = TiffLoader::getImageProperties("test/GMEV5minM1OT3_0001.btf", 0);
  std::cout << std::to_string(prop.nrOfTiles) << std::endl;

  for(int n = 0; n < prop.nrOfTiles / 36; n++) {
    auto img = TiffLoader::loadImageTile("test/GMEV5minM1OT3_0001.btf", 3, n, 36);
    img *= 0.003906250;
...
{
  auto img =
      TiffLoader::loadEntireImage("test_cell/B2_0001.vsi - 001 CY7, CY5, Cy5-Cy7-FRET, DAPI_MB, BF - Z=5.tif", 4);
  img *= 0.003906250;
