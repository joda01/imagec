 ...
 *
 * @brief BS::thread_pool: a fast, lightweight, and easy-to-use C++17 thread pool library. This header file contains
 * independent utility classes that are part of the library, but are not needed to use the thread pool itself.
 */
...
public:
  ...
  // The copy constructor and copy assignment operator are deleted. The signaller works using a promise, which cannot be
  // copied.
...
public:
  ...
  // The copy and move constructors and assignment operators are deleted. The synced stream uses a mutex, which cannot
  // be copied or moved.
...
public:
  ...
  /**
   * @brief Get the number of milliseconds that have elapsed since the object was constructed or since `start()` was
   * last called, but keep the timer ticking.
   *
  ...
  /**
   * @brief Start (or restart) measuring time. Note that the timer starts ticking as soon as the object is created, so
   * this is only necessary if we want to restart the clock later.
   */
  void start()
  {
  ...
  /**
   * @brief Stop measuring time and store the elapsed time since the object was constructed or since `start()` was last
   * called.
   */
  void stop()
  {
