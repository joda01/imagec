...
public:
  ...
  ///
  static auto getData(Analyzer &analyzer, uint8_t plateId, results::WellId wellId, const MeasureChannelId &measurement,
                      Stats stats,
                                                                                 ...
                                                                                 {13, 14, 15, 16}}) -> Table
  {
    std::unique_ptr<duckdb::QueryResult> result = analyzer.getDatabase().select(
        ...
        measurement.getKey(), wellId.well.wellId);
    ...
    for(size_t n = 0; n < materializedResult->RowCount(); n++) {
      try {
        ...
        results.setData(pos.x, pos.y, TableCell{value, true});
