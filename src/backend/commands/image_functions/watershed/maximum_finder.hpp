...
public:
  ...
  cv::Mat findMaxima(cv::Mat &ip, double tolerance, bool strict, double threshold, int outputType, bool excludeOnEdges, bool isEDM);
...
private:
  ...
  std::shared_ptr<uint64_t> getSortedMaxPoints(cv::Mat &ip, cv::Mat &typeP, bool excludeEdgesNow, bool isEDM, float globalMin, float globalMax,
                                               double threshold, size_t &maxPointSize);

  void analyzeAndMarkMaxima(const cv::Mat &ip, cv::Mat &typeP, std::shared_ptr<uint64_t> maxPoints, size_t maxPointsSize, bool excludeEdgesNow,
                            bool isEDM, float globalMin, double tolerance, bool strict, int outputType, float maxSortingError);
  ...
  int processLevel(int pass, cv::Mat &ip, std::shared_ptr<int> fateTable, int levelStart, int levelNPoints, std::shared_ptr<int> coordinates,
                   std::shared_ptr<int> setPointList);
  ...
  static std::shared_ptr<int> findMaxima(std::shared_ptr<double> xx, size_t xxSize, double tolerance, bool excludeOnEdges);
  static std::shared_ptr<int> findMinima(std::shared_ptr<double> xx, size_t xxSize, double tolerance, bool excludeEdges);
  static std::shared_ptr<int> findMinima(std::shared_ptr<double> xx, size_t xxSize, double tolerance, int edgeMode);

  /// FILTER PARAMS//////////////////////////////////////////////////
  // double tolerance = 10;    ///< prominence: maximum height difference between points that are not counted as separate maxima
  //  bool strict = false;             ///< strict=off allows one maximum even if it is not higher than the prominence above all other pixels
  ...
  static const inline char ELIMINATED = static_cast<char>(64);    // marks maxima that have been eliminated before watershed

  static const inline char outputTypeMasks[] = {MAX_POINT, MAX_AREA, MAX_AREA};    ///< type masks corresponding to the output types
