...
///
void CellCounter::execute(const joda::Image &img)
{
...
///
void CellCounter::writeReport(const joda::func::ai::DetectionResults &prediction, const joda::Image &img)
{
...
void CellCounter::mergeReportInt(const std::string &rowName, joda::reporting::Table &mergeTo,
...
void CellCounter::mergeReport(const std::string &rowName, joda::reporting::Table &mergeTo,
                              const joda::reporting::Table &mergeFrom)
{
  mergeTo.setColumnNames({{0, "Count"}, {1, "Diameter [px]"}});
  if(mergeFrom.getStatistics().contains(0)) {
