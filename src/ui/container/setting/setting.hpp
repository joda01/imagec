...
public:
  ...
  void setValue(VALUE_T newValue)
    requires std::same_as<VALUE_T, int> || std::same_as<VALUE_T, uint32_t> || std::same_as<VALUE_T, uint16_t> ||
             std::same_as<VALUE_T, float> || std::same_as<VALUE_T, std::string> || std::is_enum<VALUE_T>::value
  {
    if(mLineEdit != nullptr) {
      if constexpr(std::same_as<VALUE_T, std::string>) {
  ...
  ///
  uint16_t getValue()
    requires std::same_as<VALUE_T, uint16_t>
  {
    try {
      if(mLineEdit != nullptr) {
        return (uint16_t) mLineEdit->text().toUInt();
      }
      if(mComboBox != nullptr) {
        return (uint16_t) mComboBox->currentData().toUInt();
      }
      return 0;
    } catch(const std::exception &) {
      return 0;
  ...
  /// \return
  ///
  uint16_t getValueSecond()
    requires std::same_as<VALUE2_T, uint16_t>
  {
    try {
      if(mComboBoxSecond != nullptr) {
        return (uint16_t) mComboBoxSecond->currentData().toUInt();
      }
      return 0;
    } catch(const std::exception &) {
      return 0;
...
private:
             ...
             std::same_as<VALUE_T, float> || std::same_as<VALUE_T, std::string> || std::is_enum<VALUE_T>::value
  {
    ...
    if constexpr(std::same_as<VALUE_T, int> || std::same_as<VALUE_T, uint32_t> || std::same_as<VALUE_T, uint16_t> ||
                 std::same_as<VALUE_T, float>) {
