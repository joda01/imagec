    ...
    mMainFrame(mainFrame)
{
  ...
  this->mChoiceAImodel->Clear();
  auto foundOnnxFiles = joda::onnx::Onnx::findOnnxFiles();
  for(const auto &[onnxF, data] : foundOnnxFiles) {
    this->mChoiceAImodel->Append(onnxF);
  }
}
...
void PanelChannelController::refreshPreview(std::shared_ptr<DialogImageController> dialog)
{
  try {
    ...
    settings::json::ChannelSettings chs;
    chs.loadConfigFromString(getValues().dump());
    auto ret = mMainFrame->getController()->preview(chs, mPreviewImageIndex, mPreviewTileIndex);
