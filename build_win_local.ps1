param(
    [string]$USERPATH = "blzf7",
    [string]$TAG_NAME = "dev",
    [string]$CONAN_IMAGEC_ARTIFACTORY_PW = "pwd",
    [string]$WITH_CUDA = "False",
    [string]$GITHUB_WORKSPACE = "C:\Users\$USERPATH\Documents\imagec\"
)
...
function Fetch-ExternalLibs {
  ...
  if (Test-Path "C:\Users\$USERPATH\.conan2\profiles\default") {
  ...
  Copy-Item $GITHUB_WORKSPACE\conan\profile_win "C:\Users\$USERPATH\.conan2\profiles\default" -Force
...
function BuildPartial {
    Set-Location -Path "build"
    Get-ChildItem Env:
    cmake --build . --config Release --target imagec --parallel 11
    cd ..
}
...
function Pack {
  ...
  $conanLibInstallPath = "C:\Users\$USERPATH\.conan2\p"
  ...
  Copy-Item -Path "C:\Users\$USERPATH\Documents\imagec\build\build\Release\imagec.exe" -Destination "imagec.exe"
