...
#include "image_processor/channel_processor_base.hpp"
  ...
  void analyzeAllImages() override
  {
    ...
    for(const auto &imagePath : getListOfImagePaths()) {
      ...
      if(imgProperties.imageSize > MAX_IMAGE_SIZE_TO_OPEN_AT_ONCE) {
        ...
        for(uint64 idx = 0; idx < runs; idx++) {
          auto tilePart = TiffLoader::loadImageTile(imagePath, dir, idx, tilesToLoadPerRun);
          analyze(tilePart, tileReport, outFolder, filename, idx);
          setProgressActImage(idx + 1);
          if(isStopped()) {
            break;
          }
        }
