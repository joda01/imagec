...
template <class T>
concept Valid_t = std::is_same_v<T, bool> || std::is_same_v<T, joda::image::ParticleValidity>;
...
concept Value_t = std::is_same_v<T, double> || std::is_same_v<T, float> || std::is_same_v<T, int64_t> ||
                  ...
                  std::is_same_v<T, joda::image::ParticleValidity>;
...
public:
  ...
  void set(V val)
  {
    if constexpr(!std::is_same_v<V, joda::image::ParticleValidity>) {
      this->val = (double) val;
    } else {
      this->val = val;
    }
  ...
  [[nodiscard]] auto getVal() const -> const std::variant<double, joda::image::ParticleValidity> &
  {
    return val;
  }
  ...
  friend void to_json(nlohmann ::json &nlohmann_json_j, const Value &nlohmann_json_t)
  {
    ...
    } else if(std::holds_alternative<joda::image::ParticleValidity>(nlohmann_json_t.val)) {
      nlohmann_json_j["val"] = std::get<joda::image::ParticleValidity>(nlohmann_json_t.val);
    }
  }
  friend void from_json(const nlohmann ::json &nlohmann_json_j, Value &nlohmann_json_t)
  {
    if(nlohmann_json_j.at("val").is_number()) {
      ...
      nlohmann_json_t.val = (joda::image::ParticleValidity) nlohmann_json_j.at("val");
    }
  };

private:
  /////////////////////////////////////////////////////
  std::variant<double, joda::image::ParticleValidity> val;    ///< Value
};
...
public:
  ...
  {
    if constexpr(std::is_same_v<V, joda::image::ParticleValidity>) {
      objectMeta.valid = val == joda::image::ParticleValidity::VALID;
    } else {
...
public:
  ...
  void setValidity(joda::image::detect::ResponseDataValidity valid, bool invalidateAllObjects);
